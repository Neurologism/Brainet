...
public:
     ...
     */
    Dense(ActivationVariant activationFunction, std::uint32_t units, NormVariant norm);
    ~Dense() = default;
    /**
...
Dense::Dense(ActivationVariant activationFunction, std::uint32_t units)
{
    // error checks
    if(sGraph == nullptr)
    {
    ...
    std::shared_ptr<Operation> operation_ptr = std::visit([](auto&& arg) {
        // Assuming all types in the variant can be dynamically casted to Operation*
        return std::shared_ptr<Operation>(std::make_shared<std::decay_t<decltype(arg)>>(arg));}, ActivationVariant{activationFunction});
...
Dense::Dense(ActivationVariant activationFunction, std::uint32_t units, NormVariant norm)
{
    mpNorm = std::visit([](auto&& arg) {
        // Assuming all types in the variant can be dynamically casted to Operation*
